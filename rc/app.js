...
(() => {
    ...
    runs.forEach((r,ri)=>{
      ...
      if(r.pat!=null){
        ...
        b.innerHTML=
          ...
          '</span>';
        // The label adapts to the BLOCK, not the block to the label: every card carries its
        // full name plus a compact chip in data-smin ("S3"), and the container query in
        // styles.css swaps to the chip when the card is too narrow for the name — so a
        // sliver never reads as "Sectio…". aria-label and title below never abbreviate.
        const nm=b.querySelector('.sname');
        nm.textContent=name;
        nm.setAttribute('data-smin','S'+(r.pat+1));
        ...
        if(r.start===songSel){
          ...
          gr.addEventListener('pointerdown',ev=>{
            ...
            // Calibrate from the dragged block itself: its painted width IS r.bars drawn bars,
            // under any flex layout. The old strip-wide computation divided by songUsed(),
            // which ignores the silent bars the strip also draws — a ~4x hand-travel mismatch
            // on the seeded song. Measured once here, while the block is still attached;
            // songResize re-renders the strip and detaches this node mid-drag.
            const base=r.bars, x0=ev.clientX;
            const perBar=Math.max(1,b.getBoundingClientRect().width/Math.max(1,base));
            let last=base;
            const mv=e2=>{
        ...
        // Only the TRAILING silent run bends "width IS duration": the seeded song is 8 bars
        // of Intro plus 24 silent, so true proportion gave three quarters of the strip to a
        // dotted nothing and crushed every real card — in exactly the arrangements beginners
        // start with. The bar count stays stated in words, so the silence is still told;
        // a silent gap BETWEEN sections keeps full proportion, because that gap is structure.
        if(ri===runs.length-1){ b.style.flexGrow=String(Math.min(r.bars,4)); b.classList.add('s-tail'); }
      }
  ...
  const APP_VERSION='13.8.0-rc.11';       // semantic app version — the build that wrote the file
  ...
  // The welcome and the create sheet are dialogs too — class .welcome, shown with .on rather
  // than [hidden]. Before they were counted here, Space on a focused welcome chip ALSO toggled
  // the transport behind the modal, R armed a recording, and 1-4 switched workspace tabs.
  function modalOpen(){ return !!document.querySelector('.modal:not([hidden]),.msheet:not([hidden]),.welcome.on'); }
  ...
  function trapTab(box,e){
    // a[href], not bare [href]: the attribute selector also matches SVG <use href> — an
    // unfocusable element whose offsetParent is undefined (SVG), so it slips the visibility
    // filter, becomes `first`, and the wrap silently focuses nothing. The welcome's inline
    // logo exposed this; every dialog with a leading icon was one <use> away from the same.
    const f=[...box.querySelectorAll('button,input,select,textarea,a[href],[tabindex]:not([tabindex="-1"])')]
  ...
  // ---------- URL ingest (13.7) ----------
  // THE LAW, as amended with 13.7 and enforced by fixtures/url-ingest-qa.html and the source
  // scan in music-knowledge-qa: Aura never phones anywhere on its own. The ONLY network
  // request it may ever make is the exact URL the singer pasted, at the moment they press
  // Load. This function is the single sanctioned fetch site in the runtime; the scan counts.
  // The URL is never stored — not in the project, not in the autosave, not in a recent list.
  let URL_MAX_BYTES=100*1024*1024;   // let, not const: url-ingest-qa lowers it to prove the cap
  // Streaming platforms are refused BEFORE any request leaves the machine: physics (DRM,
  // manifests, CORS) and principle — an import is a muted reference, and Aura is not a ripper.
  const URL_REFUSED_HOSTS=['youtube.com','youtu.be','music.youtube.com','open.spotify.com',
    'spotify.com','music.apple.com','soundcloud.com','on.soundcloud.com','tidal.com',
    'listen.tidal.com','deezer.com','pandora.com'];
  const URL_FALLBACK='Download the file, then use Import — the result is identical.';
  function urlRefusal(u){
    let p; try{ p=new URL(String(u||'').trim()); }catch(e){ return 'That doesn’t look like a web address.'; }
    if(p.protocol!=='http:'&&p.protocol!=='https:') return 'Only http and https links can be loaded.';
    const host=p.hostname.toLowerCase();
    if(URL_REFUSED_HOSTS.some(h=>host===h||host.endsWith('.'+h)))
      return 'Links from streaming services can’t be imported. Aura works from files you have the right to use.';
    if(/\.(m3u8|mpd)($|\?)/i.test(p.pathname))
      return 'That’s a streaming manifest, not an audio file. '+URL_FALLBACK;
    return null;
  }
  let urlFetchCtl=null;   // lets the Cancel press abort a stalled socket, not just orphan it
  async function loadSampleUrl(url){
    const refusal=urlRefusal(url);
    if(refusal){ smpStatus(refusal); return; }
    cancelImportJob();
    const job=impJob;
    const ctl=new AbortController(); urlFetchCtl=ctl;
    let name='linked audio';
    try{ const seg=new URL(url).pathname.split('/').filter(Boolean).pop();
         if(seg) name=decodeURIComponent(seg); }catch(e){}
    smpStatus('Fetching '+name+'…');
    let resp;
    try{ resp=await fetch(url,{signal:ctl.signal,credentials:'omit',redirect:'follow'}); }
    catch(e){
      if(jobLost(job)) return;
      smpStatus('Couldn’t read that link — the host refuses browser access, or it’s offline. '+URL_FALLBACK);
      return;
    }
    if(jobLost(job)){ try{ctl.abort();}catch(e){} return; }
    if(!resp.ok){
      const s=resp.status;
      smpStatus(s===401||s===402||s===403
        ? 'That link is private. Aura can only reach what you can reach without signing in.'
        : 'The host answered '+s+' for that link. Check the address, or '+URL_FALLBACK.toLowerCase());
      return;
    }
    const declared=+(resp.headers.get('content-length')||0);
    if(declared>URL_MAX_BYTES){ try{ctl.abort();}catch(e){}
      smpStatus('That file is over 100 MB. '+URL_FALLBACK); return; }
    const ctype=(resp.headers.get('content-type')||'').split(';')[0].trim().toLowerCase();
    // A page, not a file: best-effort. When the page itself is readable, follow the single
    // media URL it declares — through this same gate, so a refusal still refuses.
    if(ctype==='text/html'||ctype==='application/xhtml+xml'){
      let text=''; try{ text=await resp.text(); }catch(e){}
      if(jobLost(job)) return;
      const m=text.match(/property=["']og:audio(?::secure_url)?["'][^>]*content=["']([^"']+)["']/i)
            ||text.match(/content=["']([^"']+)["'][^>]*property=["']og:audio(?::secure_url)?["']/i)
            ||text.match(/<audio[^>]*\ssrc=["']([^"']+)["']/i)
            ||text.match(/<source[^>]*\ssrc=["']([^"']+\.(?:mp3|wav|m4a|aac|flac|ogg)(?:\?[^"']*)?)["']/i)
            ||text.match(/href=["']([^"']+\.(?:mp3|wav|m4a|aac|flac)(?:\?[^"']*)?)["']/i);
      if(m){ let media=m[1];
        try{ media=new URL(media,url).href; }catch(e){}
        smpStatus('That page points at audio — following it…');
        return loadSampleUrl(media); }
      smpStatus('That link is a page, and Aura couldn’t find audio on it. Paste a direct audio link, or '+URL_FALLBACK.toLowerCase());
      return;
    }
    let blob;
    try{
      if(resp.body&&resp.body.getReader){
        ...
        for(;;){
          ...
          if(got>URL_MAX_BYTES){ try{ctl.abort();}catch(e){}
            smpStatus('That file is over 100 MB. '+URL_FALLBACK); return; }
          const mb=n=>(n/1048576).toFixed(1);
          smpStatus(declared>0
            ? 'Fetching '+name+' · '+mb(got)+' MB of '+mb(declared)+' MB'
            : 'Fetching '+name+' · '+mb(got)+' MB');
        }
        blob=new Blob(chunks,{type:ctype||'application/octet-stream'});
      } else {
        blob=await resp.blob();
        if(jobLost(job)) return;
        if(blob.size>URL_MAX_BYTES){ smpStatus('That file is over 100 MB. '+URL_FALLBACK); return; }
      }
    }catch(e){
      if(!jobLost(job)) smpStatus('The connection dropped while fetching. Try again, or '+URL_FALLBACK.toLowerCase());
      return;
    }
    if(jobLost(job)) return;
    urlFetchCtl=null;
    // From here it IS a local import: same decode, same MIN_MEDIA_SECONDS, same analysis,
    // same muted-reference rights model, same Preview→Apply. Zero new downstream surface.
    return loadSampleFile(new File([blob],name,{type:blob.type}));
  }
  ...
  function wireSamplePanel(){
    ...
    refFileInput=fi;
    // URL ingest wiring. Load becomes Cancel for the whole journey — fetch, decode, analyse —
    // because a singer cancelling does not care which stage the machine happens to be in.
    const urlIn=document.getElementById('urlIn'), urlLoad=document.getElementById('urlLoad');
    if(urlIn&&urlLoad){
      let urlBusy=false;
      const go=()=>{
        if(urlBusy){ cancelImportJob(); if(urlFetchCtl){ try{urlFetchCtl.abort();}catch(e){} }
          smpStatus('Cancelled.'); return; }
        const u=(urlIn.value||'').trim(); if(!u){ urlIn.focus(); return; }
        urlBusy=true; urlLoad.textContent='Cancel';
        Promise.resolve(loadSampleUrl(u))
          .finally(()=>{ urlBusy=false; urlLoad.textContent='Load'; urlFetchCtl=null; });
      };
      urlLoad.addEventListener('click',go);
      urlIn.addEventListener('keydown',e=>{ if(e.key==='Enter'){ e.preventDefault(); if(!urlBusy) go(); } });
    }
  ...
  // While the welcome covers the studio, everything behind it goes inert — aria-modal is an
  // assertion to assistive tech, not a browser behaviour, and by the time wireWelcome runs
  // mountShell has moved every focusable surface (transport, tabs, rails, Ask Aura) inside
  // #app, so that one container is the whole "behind". aria-hidden is the fallback where
  // inert is unsupported; the Tab trap in wireWelcome is the keyboard fallback there.
  let wPrevFocus=null;
  function setWelcomeInert(on){
  ...
  function closeWelcome(){ const w=document.getElementById('welcome');
    if(w) w.classList.remove('on'); setWelcomeInert(false);
    if(wPrevFocus&&wPrevFocus.isConnected){ try{ wPrevFocus.focus(); }catch(e){} }
    wPrevFocus=null;
    try{ localStorage.setItem('aura-seen','1'); }catch(e){} }
  function wireWelcome(){
    ...
    // trapTab is the same trap the name/recents/guide dialogs already use. Without it, Tab
    // from the last control walked out of the dialog and landed on #play behind the modal.
    w.addEventListener('keydown',e=>{ if(e.key==='Escape'){ close(); } else if(e.key==='Tab'){ trapTab(w,e); } });
    // The inert state is DERIVED from the DOM, not trusted to closeWelcome(): the QA suites
    // (and any future code path) dismiss these dialogs by stripping the class directly, and
    // an inert flag that only closeWelcome() clears would wedge the whole app inert. The
    // synchronous set/clear in the open path and closeWelcome() still run first, so focus
    // restoration never races the observer; this is the backstop, not the mechanism.
    const syncInert=()=>setWelcomeInert(!!document.querySelector('.welcome.on'));
    const mo=new MutationObserver(syncInert);
    document.querySelectorAll('.welcome').forEach(el=>mo.observe(el,{attributes:true,attributeFilter:['class']}));
    let seen=null; try{ seen=localStorage.getItem('aura-seen'); }catch(e){}
    if(!seen){ wPrevFocus=document.activeElement; w.classList.add('on'); setWelcomeInert(true);
      const first=w.querySelector('.wopt'); if(first) first.focus(); }
  }
  ...
  function mountShell(){
    ...
    mid.appendChild(ms);
    // No hideSm class: no selector by that name exists anywhere, and stamping a dead class on
    // the wrappers holding #bpm/#swing (which the scheduler reads every 25ms) invited a future
    // `.hideSm{display:none}` written in good faith to kill the transport with no error.
    [...oldHeader.querySelectorAll('.ctrl')].forEach(c=>{ right.appendChild(c); });
    ...
    function fitSteps(){
      ...
      for(const p of plans){
        ...
        // Ceiling 56, not 42: the solver could shrink pads on narrow viewports but never grow
        // them, which pinned the table at ~1036px while the wrap stretched to 1900 — 45% of
        // the Beat panel painted as dead interior at 2240. 56 is the piano roll's own PR_CW
        // ceiling, so pads and roll steps stay the same scale. The overhead probe above is
        // measured at 42 and overhead does not vary with pad size, so the solve stays exact.
        cell=Math.max(32,Math.min(56,cell));
        apply(p,cell);
    ...
    const xport=$('xport');
    // scrollWidth and clientWidth are integer-rounded on different subpixel edges, so at a
    // fractional browser zoom a phantom 1px "overflow" can persist at ANY width — and since
    // every escalation step below re-asks the same lying comparison, the cascade runs to full
    ...
    function reflowTransport(){
      ...
      // ~130px of recoverable buttons kept their seats. Actions go first now; labels last.
  ...
  window.__auraSuite=Object.freeze({
    // URL ingest (13.7): the fixture drives the real front door and lowers the byte cap to
    // prove the oversize refusal without a 100 MB fixture. urlProbe exposes the refuse-early
    // mapper so every refusal sentence is testable without a network request.
    urlIngest(u){ return loadSampleUrl(u); },
    urlProbe(u){ return urlRefusal(u); },
    _setUrlCap(n){ URL_MAX_BYTES=n; },
