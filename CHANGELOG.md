...
### What a second, adversarial look found

Five defects were carried as open. An independent pass re-checked each one with the instruction to
REFUTE it: two were already fixed, one was fixed but had left a new defect behind, and two were real.
It also found several things the project claimed to have built and had not.

**Kept fader moves now render into the export.** They were dropped — and the failure was worse than
dropping them. Playing back a take leaves the faders wherever the last move put them, and nothing put
them back, so the export, which reads each fader once, baked that residual position across the whole
song. A take that swept the crossfade from verse to chorus exported with *every* verse at full-chorus
settings, while the mute moves in the same take rendered correctly and made it look like automation
had worked. The replay now runs the same actions the live path runs and stamps the result onto the
export graph, so there is no second set of gains to drift. Tempo is deliberately excluded, because the
render fixes its step grid before it begins.

**A section built from an imported file called itself Aura's own.** The provenance recorder had no
caller anywhere in the product — the sampler recorded nothing at all. So a file someone else owns
could be chopped into slices, built into a section, and the project would still report *"Generated by
Aura. Yours to use."* and confirm clean. It was clean because the file was invisible to the report,
not because the file was clear. Every sampler source is now recorded as it arrives, a section built
from its slices is recorded as a transformation naming that source, and a derived part inherits the
strictest rights class in its chain: an unknown source blocks confirmation through everything made
from it, while Aura's own tone stays clear.

**Six section-name boxes became twenty-four.** The builder never cleared its host and runs on every
song-scoped restore. Not cosmetic — reloading then wrote the names across all of them and left most
blank while the project still held the right six.

**The Tool Router reached one of its ten entries.** It looked up a property nothing ever set, so every
routed question fell through to the same generic answer. It now sees the question, and searches only
the tools knowledge — craft entries load first and ties keep their order, which had *"what platform
should I put this on"* answered with a song arrangement.

**Find a sound had no route in the default mode.** Guided hides the workspace tabs on desktop, and the
step rail does not cover the Sound view. It was reachable only sideways — through Create something, an
import, or Ask Aura. It now sits in the More sheet beside Balance.

Also corrected: a parked version was restored without the sanitiser its siblings went through, so a
corrupted file could put an out-of-range section index into the arrangement and make the export throw;
the Guide's *Open Rights & Sources* highlighted the Mix Check card; Mix Check named two controls,
Punch and Width, that do not exist, under a card promising that every warning names the control that
fixes it; and four knowledge entries and seven ledger rows described features that were never built.
Those rows now read partial or not built, with the reason.
